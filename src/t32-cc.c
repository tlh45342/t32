...
typedef struct {
    ...
    int member_base_is_pointer; /* EXPR_STRUCT_MEMBER: -> instead of . */
} expression_node_t;
...
typedef struct {
    ...
    int member_base_is_pointer;/* STMT_STORE_MEMBER: -> instead of . */
    ...
    int *statement_index_out)
{
    ...
    if (parser->current.kind == TOK_IDENTIFIER &&
        ...
        statement.member_base_is_pointer = 1;
        parser_advance(parser);
        parser_advance(parser); /* '->' */
...
static int parse_function(parser_t *parser, program_t *program)
{
    ...
    if (token_is_identifier(&parser->current, "void")) {
        ...
        for (;;) {
            ...
            if (program->parameter_count >= 4) {
                parser_error(parser, "Stage 24 supports at most four parameters");
                return 0;
            }
            if (!parser_expect_word(parser, "int")) return 0;
            if (parser->current.kind != TOK_IDENTIFIER) {
    ...
    if (program->is_main && program->parameter_count != 0) {
        parser_error(parser, "Stage 24 main must have no parameters");
        return 0;
    }
    ...
    unsigned next_register)
{
    ...
    if (node->kind == EXPR_STRUCT_MEMBER) {
        ...
        if (node->member_base_is_pointer) {
            emit_operand(output,
    ...
    const loop_context_t *loop_context)
{
    ...
    if (statement->kind == STMT_STORE_MEMBER) {
        ...
        if (statement->member_is_char)
            emit_truncate_char(output, 1);
        if (statement->member_base_is_pointer) {
            emit_operand(output,
