...
# T32 Platform Checkpoint — August 11, 2026

The compiler history above is preserved as part of the T32 development record.
The larger T32 platform has now advanced beyond isolated compiler and ISA work
into a small, bootable computer with firmware, runtime services, storage, an
interactive C monitor, and both command-line and graphical VM hosts.
...
The complete BIOS -> BOOT.BIN -> NEXT.BIN path is validated.

Current firmware/runtime milestones include:

- T32 BIOS 0.0.6;
- T32 BOOT 0.0.4;
- Bootinfo v0.2;
- BIOS disk service v0.1;
- compiler-built Stage3 monitor 0.0.13;
- `libt32` static target runtime;
- virtual block storage and T32D boot media;
- guest keyboard and 80x25 text display;
- guest-visible RTC providing host UTC epoch seconds.

The Stage3 monitor currently provides:
...
The `time` command is validated both in standalone Stage3 testing and through
the complete BIOS -> BOOT -> NEXT boot chain.

## t32-runx

`t32-runx` is the Windows graphical T32 developer VM.

The current version supports interactive display and keyboard operation, disk
attachment, firmware controls, and an embedded default BIOS. The embedded BIOS
makes the normal graphical startup path deliberately simple: attach the T32
disk image and start the machine. An external BIOS remains available as an
override for firmware development and testing.

![t32-runx booting the T32 Stage3 monitor](update.png)

The longer-term intent is for this simplicity to become part of a machine
configuration/JSON model describing memory, firmware selection, disks, and
other virtual hardware.

## Next development sprints

The next useful step is to make Stage3 capable of loading and executing a
separate application rather than replacing Stage3 itself.
...
application-facing SVC ABI
    |
monotonic timing / benchmark support
    |
...
An external development directory such as `G:\X2` is useful for this work
because it proves that installed T32 tools and libraries are sufficient to
build software outside the T32 source tree. Mature examples can later be moved
into `samples/`.

## SVC direction

As external applications become real, application code should not need to know
raw MMIO addresses or BIOS implementation details.
...
Likely early services include console output, application exit/return, wall
clock access, monotonic timing, and disk reads.

The existing BIOS service table remains a firmware/bootstrap interface. It does
not need to become the application ABI.

## Timing

The RTC now supplies wall-clock UTC time.

Benchmarking and elapsed-time measurement should use a separate monotonic
timer/counter. This will give workloads such as SORT.BIN a stable way to report
execution time without confusing wall-clock time with performance timing.

## Filesystem direction

T32D remains the intentionally simple bootstrap disk format.

A real filesystem is a later layer, with ext2 a strong target once external
program execution, the SVC boundary, and timing are established. This provides
a natural path from a bootstrap image toward a machine that can locate and load
ordinary files.

## C library and header direction

As `t32-cc` and `libt32` mature, normal target headers should accompany the
runtime. Programs should eventually be able to write conventional source such
as:
...
and inherit the appropriate T32 prototypes and declarations from the installed
target environment rather than manually reproducing them in each program.

Candidate foundational headers include:
...
`t32-run` and `t32-runx` intentionally remain single-vCPU reference/developer
machines.

Multi-VM lifecycle, machine management, and reporting belong in the later
`t32-node` architecture and its integration with Foundry. A future machine
description can make startup progressively simpler while keeping the underlying
BIOS, boot, storage, and VM contracts explicit and testable.
