...
def main() -> int:
    print("Running hex-to-string validation...\n")
    ...
    if not re.search(r"loaded\s+hex-to-string\.bin\s+at\s+0x0*1000", output, re.I):
    ...
    require_register(output, 8, 0, "zero converts to eight zeroes")
    require_register(output, 9, 0, "hexadecimal A converts correctly")
    require_register(output, 10, 0, "leading zeroes are retained")
    require_register(output, 11, 0, "mixed hexadecimal digits convert correctly")
    require_register(output, 12, 0, "all-ones value converts correctly")
