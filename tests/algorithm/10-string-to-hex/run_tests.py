...
def main() -> int:
    print("Running string-to-hex validation...\n")
    ...
    if not re.search(
        r"loaded\s+string-to-hex\.bin\s+at\s+0x0*1000",
        output,
        re.I,
    ):
    ...
    require_register(output, 12, 0xDEADBEEF, "lowercase hexadecimal parses correctly")
    require_register(output, 13, 1, "short input is rejected")
