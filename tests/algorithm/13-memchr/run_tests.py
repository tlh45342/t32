...
def register_value(text: str, register: int) -> int | None:
    match = re.search(rf"\br{register}\s*=\s*(0x[0-9a-fA-F]+|\d+)\b", text)
...
def require_register(text: str, register: int, expected: int, label: str) -> None:
    ...
    if actual != expected:
        fail(
            f"{label}: expected r{register}=0x{expected:08X}, got 0x{actual:08X}",
            text,
        )
    passed(label)
...
def main() -> int:
    print("Running memchr validation...\n")
    ...
    if not re.search(r"loaded\s+memchr\.bin\s+at\s+0x0*1000", output, re.I):
