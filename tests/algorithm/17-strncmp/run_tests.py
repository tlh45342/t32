...
def main() -> int:
    print("Running strncmp validation...\n")
    ...
    for pattern, label in (
        (r"loaded\s+strncmp\.bin\s+at\s+0x0*1000", "binary loaded"),
    ...
    require_register(output, 8, 0, "zero-count comparison is equal")
    require_register(output, 9, 0, "empty strings compare equal")
    require_register(output, 10, 0, "identical strings compare equal")
    require_register(output, 11, 0xFFFFFFFF, "mismatch inside limit is detected")
    require_register(output, 12, 0, "mismatch beyond limit is ignored")
    require_register(output, 13, 0xFFFFFF9D, "terminator difference is detected")
